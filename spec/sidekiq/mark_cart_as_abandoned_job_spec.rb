...
RSpec.describe MarkCartAsAbandonedJob, type: :job do
  describe '#perform' do
    ...
    context 'when there is not an abandoned cart' do
      let!(:active_cart) { Cart.create!(total_price: 0) }
    ...
    context 'when there is a recent abandoned cart' do
      let!(:to_be_abandoned_cart) { Cart.create!(total_price: 0, last_interaction_at: 3.hours.ago) }
    ...
    context 'when there is a old abandoned cart' do
      let!(:to_be_removed_cart) { Cart.create!(total_price: 0, last_interaction_at: 7.days.ago, abandoned: true) }
